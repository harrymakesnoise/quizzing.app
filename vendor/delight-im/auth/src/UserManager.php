		...
		if ($requireUniqueUsername) {
			...
			if ($username !== null) {
				...
				$occurrencesOfUsername = $this->db->selectValue(
					'SELECT COUNT(*) FROM ' . $this->makeTableName('users') . ' WHERE username = ? and siteid = ?',
					[ $username, $quizSite->siteid ]
				);
	...
	protected function getUserDataByUsername($username, array $requestedColumns) {
		...
		try {
			...
			$users = $this->db->select(
				'SELECT ' . $projection . ' FROM ' . $this->makeTableName('users') . ' WHERE username = ? and (siteid = ? or siteid = ?) LIMIT 2 OFFSET 0',
				[ $username, $quizSite->siteid, 0 ]
			);
		}
		catch (Error $e) {
			throw new DatabaseError($e->getMessage());
